...
export default function CustomSearch({ search, setSearch, setUpdateData, setPage }: CustomSearchProps) {
    return (
        <div className="w-full flex relative border-1 border-primary-light bg-gray-100 dark:bg-gray-700 rounded-sm p-1 transition-all">
            ...
            />
            {search != "" && <button onClick={() => { setSearch(""), setUpdateData(Math.random()) }} className="border-r-1 hover:cursor-pointer [&>*]:hover:stroke-primary-light transition-all"><IoCloseOutline className="w-5 h-5" /></button>}
            <button
                onClick={() => {setUpdateData(Math.random()), setPage(1)}}
                disabled={search == ""}
                className={`pl-1 hover:cursor-pointer [&>*]:hover:stroke-primary-light transition-all disabled:cursor-default`}>
